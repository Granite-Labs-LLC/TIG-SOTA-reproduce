...
extern "C" __global__
void scale_fp32_m1p1_to_positive(const float* __restrict__ in,
                                 ...
                                 int n)
{
    ...
    for (; i < n; i += stride) {
        // convert -1<x<1 to 0<x<2
        out[i] = in[i] + 1.0f;
    }
}

extern "C" __global__
void scale_pad_fp32_m1p1_to_0_255f(const float* __restrict__ in,
                                   float* __restrict__ out,
                                   int num_vecs,
                                   int cur_dims,
                                   int pad_dims,
                                   float pad_value /* e.g., 127.5f */)
{
    const float A = 127.5f; // 255/2
    const int64_t nout = (int64_t)num_vecs * (int64_t)pad_dims;

    int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    int64_t stride = (int64_t)blockDim.x * gridDim.x;

    for (; i < nout; i += stride) {
        int v = (int)(i / pad_dims);   // row index
        int d = (int)(i % pad_dims);   // col index in padded row
        float y;

        if (d < cur_dims) {
            // source index in compact input
            int in_idx = v * cur_dims + d;
            // y = (x + 1) * 127.5  (scale from [-1,1] to [0,255])
            y = fmaf(in[in_idx], A, A);
            // clamp just in case
            y = fminf(fmaxf(y, 0.0f), 255.0f);
        } else {
            // pad with neutral midpoint (equivalent to input 0.0)
            y = pad_value;
        }
        out[i] = y;
    }
