...
let words_per_plane = ((dims + 63) >> 6) as usize;       // W = ceil(D/64)
let words_per_plane_i32: i32 = words_per_plane as i32;

// --- Allocate bitplane buffers: two planes (B0, B1) for DB and queries ---
...
// --- Convert packed-u2 -> bitplanes (DB and queries) ---
let u2_packed_to_bitplanes = module.load_function("u2_packed_to_bitplanes")?;

// grid for “one thread per vector”
...
unsafe {
    // DB
    stream
        .launch_builder(&u2_packed_to_bitplanes)
        .arg(&d_db_u2)             // packed [N][(D+3)>>2]
        .arg(&mut d_db_b0)         // out plane0 [N][W]
        .arg(&mut d_db_b1)         // out plane1 [N][W]
    ...
    stream
        .launch_builder(&u2_packed_to_bitplanes)
        .arg(&d_qv_u2)             // packed [M][(D+3)>>2]
        .arg(&mut d_q_b0)          // out plane0 [M][W]
        .arg(&mut d_q_b1)          // out plane1 [M][W]
...
// --- Shared memory sizing for bit-sliced Top-K ---
// Per-thread spill for the heap:
let per_thread_bytes = topk * (std::mem::size_of::<i32>() + std::mem::size_of::<f32>());

// Query bitplanes cached in shared: 2 planes * W words * 8B/word
let base_query_bytes = 2 * words_per_plane * std::mem::size_of::<u64>();
...
let cfg_bs = LaunchConfig {
    grid_dim: (challenge.difficulty.num_queries, 1, 1),
    block_dim: (threads_per_block, 1, 1),
    shared_mem_bytes: shared_mem_bytes,
};

// --- Launch bit-sliced 2-bit Top-K ---
let k_i32: i32 = topk as i32;
let find_topk_neighbors_u2_bitsliced_kernel = module.load_function("find_topk_neighbors_u2_bitsliced_kernel")?;

unsafe {
    stream
        .launch_builder(&find_topk_neighbors_u2_bitsliced_kernel)
        .arg(&d_q_b0)                 // query plane 0 [M][W]
        .arg(&d_q_b1)                 // query plane 1 [M][W]
        .arg(&d_db_b0)                // db    plane 0 [N][W]
        .arg(&d_db_b1)                // db    plane 1 [N][W]
    ...
    println!("===== stat_filter bitslice ( Top-{} ) =====", topk);
    println!(
        "Time for nonce: {} ms (sum+stats: {:.3} ms + mad_sort: {:.3} ms + prep: {:.3} ms + search: {:.3} ms)",
        elapsed_time_ms,
        elapsed_time_ms_1,
        elapsed_time_ms_2 - elapsed_time_ms_1,
        elapsed_time_ms_3 - elapsed_time_ms_2,
        elapsed_time_ms - elapsed_time_ms_3
    );

    //
    // === Re-rank Top-K on FP32 (exact L2) ===
    //

    let start_time_rerank = std::time::Instant::now();
    ...
    unsafe {
        stream
            ...
            .arg(&challenge.d_query_vectors)   // FP32 queries (original dim)
            .arg(&challenge.d_database_vectors)// FP32 DB (original dim)
            .arg(&d_topk_indices)              // [num_queries * K] (i32)
            .arg(&mut d_refined_index)         // OUT best per query
            .arg(&mut d_refined_distance)      // OUT best distance per query
            .arg(&challenge.difficulty.num_queries)
            .arg(&challenge.vector_dims)       // original dim (e.g., 128)
            .arg(&k_i32)
            .launch(cfg_refine)?;
    }
    stream.synchronize()?;
    ...
    let elapsed_time_rerank_ms = start_time_rerank.elapsed().as_micros() as f32 / 1000.0;

    println!(
        "Time to re-rank top {} closest: {:.3} ms)",
        topk,
        elapsed_time_rerank_ms
    );
