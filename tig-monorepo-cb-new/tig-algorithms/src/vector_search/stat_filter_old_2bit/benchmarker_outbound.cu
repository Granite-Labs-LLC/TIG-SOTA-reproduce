    ...
    int dims)
{
    ...
    //int   bin    = __float2int_rn(y / sj);   // nearest (y / s[d])
#ifdef USE_4_BITS
...
)
{
    ...
    for (int i = threadIdx.x; i < vector_database_len; i += blockDim.x) {
        ...
        #pragma unroll 16
        for (int by = 0; by < row_bytes; ++by) {  // ONLY if vector_size%8 == 0
            int j = by << 2;          // j = 4*by
            uint8_t b = drow[by];
        ...
        } else { // vector_size not even multiple of 4
...
// fastest_top1_u2_packed.cu
#include <cuda.h>
#include <cuda_runtime.h>
#include <float.h>   // FLT_MAX
#include <math.h>

extern "C" __global__ void find_best_neighbor_u2_packed_kernel(
    const uint8_t* __restrict__ query_vectors_packed,   // [M][(D+3)>>2]
    const uint8_t* __restrict__ vector_database_packed, // [N][(D+3)>>2]
    const float*   __restrict__ norm_l2,                // [N]
    const float*   __restrict__ norm_l2_squared,        // [N]
    int*           __restrict__ best_index_out,         // [M]
    float*         __restrict__ best_distance_out,      // [M] (squared L2)
    const float    max_distance,
    const int      vector_database_len,                 // N
    const int      query_vectors_len,                   // M
    const int      vector_size,                         // D
...
)
{
    ...
    // [0 .. blockDim.x-1]          -> per-thread best distance
    // [blockDim.x .. 2*blockDim.x) -> per-thread best index (reinterpreted as float*)
    // [then]                       -> unpacked query codes: D bytes (0..3)
    extern __shared__ unsigned char smem[];
    float* sm_best_dist = reinterpret_cast<float*>(smem);
    int*   sm_best_idx  = reinterpret_cast<int*>(sm_best_dist + blockDim.x);
    uint8_t* sm_qv      = reinterpret_cast<uint8_t*>(sm_best_idx + blockDim.x);
    ...
    // --- Unpack the query's 2-bit codes into sm_qv (bytes 0..3) ---
    const int row_bytes = (vector_size + 3) >> 2;
    const uint8_t* qrow = query_vectors_packed + (size_t)q * row_bytes;
    ...
    // --- Per-thread scan over its strided rows ---
    ...
    for (int i = threadIdx.x; i < vector_database_len; i += blockDim.x) {
        ...
        // Compute dot(query, db) with dp4a (4 bytes per op). Our codes are 0..3, safe for int8.
        float dot;
        {
#if __CUDA_ARCH__ >= 610
            int acc_i = 0;
            ...
            if ((vector_size & 3) == 0) {
                ...
                for (int by = 0; by < row_bytes && j < vector_size; ++by, j += 4) {
                    ...
                    int q0 = (j     < vector_size) ? (int)sm_qv[j    ] : 0;
                    int q1 = (j + 1 < vector_size) ? (int)sm_qv[j + 1] : 0;
                    int q2 = (j + 2 < vector_size) ? (int)sm_qv[j + 2] : 0;
                    int q3 = (j + 3 < vector_size) ? (int)sm_qv[j + 3] : 0;

                    int q_pack =  q0 | (q1 << 8) | (q2 << 16) | (q3 << 24);
                    ...
                    int db_pack = d0 | (d1 << 8) | (d2 << 16) | (d3 << 24);
            ...
            dot = (float)acc_i;
#else
#  error "CUDA architecture too old for dp4a"
            // Fallback (not expected to be compiled on pre-SM61):
            float acc = 0.0f;
            int j = 0;
            for (int by = 0; by < row_bytes; ++by) {
                uint8_t b = drow[by];
                if (j < vector_size) { acc = fmaf((float)sm_qv[j], (float)((b      ) & 0x3), acc); ++j; }
                if (j < vector_size) { acc = fmaf((float)sm_qv[j], (float)((b >> 2) & 0x3), acc); ++j; }
                if (j < vector_size) { acc = fmaf((float)sm_qv[j], (float)((b >> 4) & 0x3), acc); ++j; }
                if (j < vector_size) { acc = fmaf((float)sm_qv[j], (float)((b >> 6) & 0x3), acc); ++j; }
            }
            dot = acc;
#endif
        }
        ...
        if ((max_distance <= 0.0f || d2 <= max_distance) && d2 < best_d) {
            best_d = d2;
            best_i = i;
        }
    }

    // Write per-thread bests into shared arrays and reduce
    sm_best_dist[threadIdx.x] = best_d;
    sm_best_idx [threadIdx.x] = best_i;
    __syncthreads();

    for (int stride = blockDim.x >> 1; stride > 0; stride >>= 1) {
        if (threadIdx.x < stride) {
            float other_d = sm_best_dist[threadIdx.x + stride];
            int   other_i = sm_best_idx [threadIdx.x + stride];
            if (other_d < sm_best_dist[threadIdx.x]) {
                sm_best_dist[threadIdx.x] = other_d;
                sm_best_idx [threadIdx.x] = other_i;
            }
        }
        __syncthreads();
    }
    ...
    if (threadIdx.x == 0) {
        best_index_out[q]    = sm_best_idx[0];
        best_distance_out[q] = sm_best_dist[0];
    }
}
