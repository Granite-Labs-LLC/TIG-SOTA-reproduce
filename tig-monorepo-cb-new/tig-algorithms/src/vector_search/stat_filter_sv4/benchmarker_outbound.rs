...
pub fn solve_challenge(
    ...
    let mut bitmode = read_bitmode();
    if bitmode != 2 && bitmode != 4 { // must be 2 or 4
        bitmode = 4;
    }
...
*/
    ...
    println!("===== stat_filter bitslice {}-bit ( Top-{} ) =====", bitmode, topk);
    println!(
...
fn read_bitmode() -> usize {
    env::var("STATFILT_BIT_MODE")
        .ok()
        .and_then(|s| s.trim().parse::<usize>().ok())
        .filter(|&v| v > 0)
        .unwrap_or(DEFAULT_BIT_MODE)
}
