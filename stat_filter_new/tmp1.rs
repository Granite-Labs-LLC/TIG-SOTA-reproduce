...
pub fn solve_challenge(
    ...
    let compute_norms_u4_bitsliced_kernel = module.load_function("compute_norms_u4_bitsliced_kernel")?;
    let mut d_db_norm_l2        = stream.alloc_zeros::<f32>(n_db)?;
    let mut d_db_norm_l2_sq     = stream.alloc_zeros::<f32>(n_db)?;
    let mut d_query_norm_l2     = stream.alloc_zeros::<f32>(n_q)?;
    let mut d_query_norm_l2_sq  = stream.alloc_zeros::<f32>(n_q)?;

    unsafe {
        // DB norms
        stream
            .launch_builder(&compute_norms_u4_bitsliced_kernel)
            .arg(&d_db_b0).arg(&d_db_b1).arg(&d_db_b2).arg(&d_db_b3)
            .arg(&mut d_db_norm_l2).arg(&mut d_db_norm_l2_sq)
            .arg(&(challenge.database_size))   // N
            .arg(&(challenge.vector_dims))     // D
            .arg(&words_per_plane_i32)         // W
            .launch(LaunchConfig { grid_dim: grd_db, block_dim: blk_conv, shared_mem_bytes: 0 })?;
        ...
        stream
            ...
            .arg(&mut d_query_norm_l2).arg(&mut d_query_norm_l2_sq)
            .arg(&(challenge.difficulty.num_queries))   // M
            .arg(&(challenge.vector_dims))              // D
            .arg(&words_per_plane_i32)                  // W
            .launch(LaunchConfig { grid_dim: grd_q, block_dim: blk_conv, shared_mem_bytes: 0 })?;
...
unsafe {
    stream
        ...
        .arg(&d_db_norm_l2_sq)        // bin-space norms^2      [N]
        ...
        .arg(&d_query_norm_l2_sq)      // bin-space query norms^2      [M]
        .arg(&words_per_plane_i32)     // W
        .launch(cfg_topk)?;
}
...
stream.synchronize()?;
    ...
    println!(
        "Time for nonce: {:.3} ms (sum+stats: {:.3} ms + mad_sort: {:.3} ms + slice: {:.3} ms + search: {:.3} ms)",
        elapsed_time_ms,
        elapsed_time_ms_1,
